...
class AuthServiceTest {
    ...
    /**
     * Test to verify that the register method of the AuthService service correctly registers users.
     * This test simulates the user registration process and verifies that the returned token is not null and matches the expected one.
     */
    @Test
    void testRegister() {
        // Arrange
        ...
        when(jwtService.generateToken(eq(registerRequest.getEmail()), any(Timestamp.class))).thenReturn(token);
        when(unverifiedUserRepository.save(any(UnverifiedUser.class))).thenReturn(unverifiedUser);

        // Mock the generateVerificationCode method
        AuthService spyAuthService = spy(authService);
        doReturn(verificationCode).when(spyAuthService).generateVerificationCode();

        // Act
        AuthResponse response = spyAuthService.register(registerRequest);
