package com.sunflowers.ecommerce.utils;

public class FrontLinks {

    public static final String MAIN_URL = "http://localhost:5173/bootcamp-tech-titans-2024_ecommerce";
    public static final String VERIFICATION_CODE = MAIN_URL + "/register/verification-code";
    public static final String PAYPAL_RETURN = MAIN_URL + "/checkout/payment";
    public static final String RESET_PASSWORD = MAIN_URL + "/account/reset-password";
}